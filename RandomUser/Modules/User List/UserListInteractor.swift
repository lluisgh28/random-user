...
class UserListInteractor: UserListInteractorInterface {
    ...
    private func filter(user: User, withText text: String) -> Bool {
        ...
        return user.firstName?.localizedCaseInsensitiveContains(text) ?? false
            || user.lastName?.localizedCaseInsensitiveContains(text) ?? false
            || user.email?.localizedCaseInsensitiveContains(text) ?? false
    }
